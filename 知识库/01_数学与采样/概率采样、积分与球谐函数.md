...
输入 `weights` 是非负函数值，`cdf` 保存归一化前缀和。采样返回离散索引及其概率密度：
...
Sample DrawFromCdf(Span<float> weights, Span<float> cdf, float u)
{
    ...
    int index = LowerBound(cdf, u * total);
    return { index, weights[index] / total };
}
```

估计积分时，样本贡献需要除以返回的 PDF。所有权重为零时，示例回退到均匀采样；调用方也可以跳过该分布。可用常量函数验证：只要 PDF 与采样分布一致，多次估计的均值都应接近解析结果。
