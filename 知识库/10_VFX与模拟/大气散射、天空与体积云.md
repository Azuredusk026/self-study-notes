# 大气散射、天空与体积云

天空和云都属于**参与介质（Participating Media）**渲染。光在介质中传播时会被吸收、被散射到别处，也会从别的方向散射进当前视线。它们不是给背景贴一张渐变图，而是在有限预算内近似辐射传输。

## 消光、透射与入射散射

介质密度为 rho，吸收系数和散射系数分别为 sigma_a、sigma_s。总消光系数是 sigma_t = sigma_a + sigma_s。光沿长度 s 传播后的透射率为：

$$
T(s)=\exp\left(-\int_0^s \sigma_t(x)\,dx\right)
$$

这就是 Beer-Lambert 定律。密度均匀时可写成 exp(-sigma_t s)。Ray Marching 会把路径分成若干小段，在每段估计透射率和入射散射，再从近到远累积。步长太大时会出现分层和漏光，步长太小时成本会直接随采样数增长。

相函数 p 描述光从入射方向散射到观察方向的比例。**瑞利散射**主要解释小粒子对短波长光的强散射，天空因此偏蓝；**米氏散射**更适合描述气溶胶和较大粒子，方向性更强，会形成太阳附近的亮区。Henyey-Greenstein 相函数常用参数 g 控制前向或后向散射。g 不是“云的亮度”，而是散射方向分布。

## 实时大气

直接对每个像素同时积分视线和太阳光路很昂贵。实时方案通常把稳定维度预计算进 LUT：

- Transmittance LUT 查询点到大气边界的透射率；
- Multiple Scattering LUT 近似高阶散射贡献；
- Sky View LUT 按观察位置和方向给出天空辐亮度；
- Aerial Perspective 数据描述远处物体被雾化和染色的程度。

LUT 的坐标映射必须和大气几何一致。行星半径、观察高度和太阳天顶角的参数化一旦不匹配，地平线附近最容易出现接缝。太阳圆盘应作为有限角半径的光源处理，并和大气透射相乘，不能只在天空颜色上叠一个白点。

## 体积云

体积云先由 Weather Map 决定大尺度覆盖、云型和高度，再用 Perlin、Worley 等噪声雕刻细节。采样点的密度通常还会经过高度曲线调整，使云底、云体和云顶具有不同形态。

每个视线采样点需要估计朝向主光的透射率。完整二次积分过贵，因此会减少光照步数、使用锥形采样或缓存。云边银边来自强前向散射和较短的光路，不应只靠 Emissive 硬提亮。

性能优化首先减少无效步进：

- 用包围体求进入和离开距离；
- 空密度区域使用更大步长；
- 远处降低采样数和分辨率；
- 对结果做时域重投影，并对相机切换和遮挡变化拒绝历史；
- 对蓝噪声抖动后的结果做稳定过滤，避免固定条带。

半分辨率渲染能明显降低成本，但上采样必须参考深度，云与几何交界处还要处理历史失效。优化时同时观察采样数、屏幕覆盖率、历史稳定性和太阳方向变化。

### 体积步进中的透射率

下面的 HLSL 沿相机射线积累单次散射。输入密度单位与消光系数必须匹配，`stepLength` 使用世界距离：

```hlsl
float3 radiance = 0.0;
float transmittance = 1.0;
for (uint i = 0; i < stepCount; ++i) {
    float3 p = rayOriginWS + rayDirWS * ((i + 0.5) * stepLength);
    float density = SampleCloudDensity(p);
    float extinction = density * extinctionPerMeter;
    float stepT = exp(-extinction * stepLength);
    radiance += transmittance * (1.0 - stepT)
              * lightRadiance * PhaseHG(dot(rayDirWS, lightDirWS), g);
    transmittance *= stepT;
    if (transmittance < 0.01) break;
}
```

用 `1-stepT` 表示本步损失的能量，步长变化时结果比简单累加密度稳定。示例省略了到光源的阴影透射和多次散射。把步数减半并把步长加倍，整体亮度应接近；差异很大时说明积分、密度频率或抖动仍依赖步长。

## Froxel 体积雾

局部体积雾的主流实时方案是把视锥离散成体素网格。由于这些体素是视锥形而非立方体（沿深度方向逐渐变大），通常称为 froxel（frustum voxel）。

典型分辨率如 $160\times90\times64$。深度方向一般按指数分布，让近处体素更密——与透视投影的采样密度需求一致。

整个方案分两步：

1. **散射计算**：每个 froxel 求一次光照散射与消光，结果写入三维纹理；
2. **积分**：沿深度方向做前缀和，得到"从相机到该深度的累计散射"。

第二步是关键。有了前缀和，着色时只需按像素深度采样一次三维纹理就能拿到总散射量，复杂度 $O(1)$，不必沿视线逐步进行光线步进。

### 沿深度复用

让一个线程负责一段连续的深度切片，而非每个 froxel 一个线程：

```hlsl
[numthreads(4, 4, 4)]
void ScatteringKernel(uint3 id : SV_DispatchThreadID)
{
    uint startSlice = id.z * _SlicesPerThread;
    uint endSlice   = startSlice + _SlicesPerThread;

    for (uint3 v = uint3(id.xy, startSlice); v.z < endSlice; ++v.z)
    {
        // froxelFront = 上一格的 froxelBack，沿射线递推
        // 距离、透射率可以累加复用
    }
}
```

沿 Z 步进时，前一个体素的远平面就是后一个的近平面，射线距离与累计透射率都可以递推。这比每个体素独立计算省下大量重复工作，代价是牺牲 Z 方向的并行度。

### 时域累积

逐帧独立计算的体积雾噪点明显，提高采样数的代价又太高。标准做法是时域累积：每帧用抖动偏移采样，再与历史结果混合。

典型的混合权重偏向历史（例如当前帧仅占七分之一），用多帧摊薄噪声：

```hlsl
float3 reprojUVW = CalcReprojectUVW(reprojPosWS, _PrevCameraPositionWS,
                                    _PrevViewProjectionMatrix, ...);
if (all(reprojUVW >= 0.0) && all(reprojUVW <= 1.0))
{
    real3 history = SAMPLE_TEXTURE3D_LOD(_PrevVolumetricFog, s_linear, reprojUVW, 0).rgb;
    current = lerp(current, history, TAA_HISTORY_BLEND_WEIGHT);
}
```

一个关键细节：**反投影必须使用未抖动的位置，采样才使用抖动位置**。两者混用会让历史查找位置随抖动摆动，表现为持续闪烁——这与抖动本身要消除的噪声是两回事，且更难察觉成因。

实现上需要双缓冲三维纹理，每帧交换当前与历史。超出历史范围（相机快速移动、首帧）时要回退到纯当前帧结果。

### 融合两个 Kernel 的可能

散射与积分分成两个 Kernel 的代价是：一整张三维纹理的写入加读取、一次额外 Dispatch、一个 UAV 屏障。

由于散射 Kernel 本来就在沿 Z 循环，加一个寄存器累加器就能在同一趟里同时输出散射与前缀和两张图。代价是丢掉 Z 方向的并行度——原本可以多个线程分担的深度切片，融合后必须由单线程串行走完。

是否划算取决于三维纹理的带宽成本与并行度损失的相对大小，需要在目标平台实测。这类"多趟换并行度"的权衡在 Compute 管线中很常见。
## 相关主题

- [[01_数学与采样/信号、频率与噪声]]
- [[02_GPU与光栅化管线/抗锯齿与时域采样]]
- [[02_GPU与光栅化管线/TAA与时域超采样]]
- [[03_Shader编程/Compute Shader与GPU执行模型]]
- [[07_颜色与后处理/颜色空间、Alpha、HDR与曝光]]
- [[10_VFX与模拟/常用VFX材质、模拟与性能]]
- [[14_性能分析与优化/帧时间、瓶颈与GPU成本]]

## 参考资料

- Sébastien Hillaire, *A Scalable and Production Ready Sky and Atmosphere Rendering Technique*.
- Eric Bruneton and Fabrice Neyret, *Precomputed Atmospheric Scattering*.
- Andrew Schneider, *The Real-time Volumetric Cloudscapes of Horizon Zero Dawn*.
- Bartlomiej Wronski, *Volumetric Fog: Unified Compute Shader Based Solution to Atmospheric Scattering*, SIGGRAPH.
- Sébastien Hillaire, *Physically Based and Unified Volumetric Rendering in Frostbite*.
