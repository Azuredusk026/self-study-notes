...
这条规则有一个例外：资产本身就以平滑后的法线作为着色法线时，外扩方向可以直接复用法线通道。植被的包裹法线烘焙就属于这种情况——法线已经被改写成体积化的朝向，描边外扩因此自动获得连续方向，不需要额外通道。前提是该资产的光照也确实按这套法线设计，而不是被描边需求倒逼修改。

### 平滑法线存在哪里

既然平滑法线必须与着色法线分开，就要找一个通道存放它。可选位置各有取舍，选择直接影响资产管线与运行时成本。

| 存放位置 | 容量 | 代价 |
|---|---|---|
| 顶点色 | 3 通道，各 8 位 | 占用顶点色，与顶点色遮罩、AO 冲突 |
| 备用 UV 通道 | 2 通道，通常 float | 需要压到两维；额外顶点属性带宽 |
| 切线通道 | 3 通道，float | 与法线贴图争用，只适合不用法线贴图的资产 |
| 贴图 | 任意精度 | 顶点阶段采样，需要唯一展开的 UV |

**顶点色**是最常见的选择。把平滑法线从 $[-1,1]$ 映射到 $[0,1]$ 写进 RGB，顶点着色器中解码即可。8 位精度对描边方向足够——方向的轻微偏差只影响描边宽度的微小变化，不会造成可见错误。

需要注意平滑法线通常存在**对象空间**，顶点着色器中直接用于位置外扩：

```hlsl
float3 smoothNormalOS = input.color.rgb * 2.0 - 1.0;
float3 positionOS = input.positionOS.xyz + smoothNormalOS * _OutlineWidth;
```

若要存切线空间，则需要一套完整的切线基底才能还原，多数情况下没有必要。

**备用 UV 通道**适合顶点色已被占用的项目。两个分量不足以直接存三维方向，需要先做两通道编码（如八面体映射）再解码。多一步计算，换来顶点色的自由。

**贴图方案**精度最高，但要求模型有唯一展开的 UV（不能有重叠），且顶点阶段的纹理采样在部分移动平台代价不低。通常只在精度确有要求时使用。

### 烘焙时要处理什么

平滑法线的生成不是简单地把相邻面法线取平均。

**按位置合并而非按顶点索引**。硬边处同一位置有多个顶点，必须先按世界位置（带容差）分组，再对组内所有顶点的法线求平均，最后把同一个结果写回组内每个顶点。只对顶点索引求平均起不到任何作用——它们本来就是分开的。

**加权方式**。简单算术平均会让三角形密集的一侧占据主导，方向偏斜。按三角形面积或顶点处的张角加权更合理，后者对不均匀网格更稳健。

**镜像与 UV 接缝**。角色资产常做左右镜像，接缝两侧的顶点位置重合但切线手性相反。按位置合并时要确认不会把手性冲突的数据混在一起。

**LOD 一致性**。每一级 LOD 都要各自烘焙。遗漏会导致 LOD 切换时描边宽度突变——这与植被包裹法线面临的是同一类问题。

烘焙应当放在导入流程中自动执行，而不是依赖美术手工操作。手工步骤在资产迭代中必然会被遗漏，且遗漏的表现（某个模型的描边偶尔裂开）很难定位。
...
技术方案决定描边"能不能画出来"，但决定"好不好看"的是宽度分布规律。多数 3D 描边显得生硬，原因不是算法精度，而是所有线条一样粗。

手绘线条的粗细变化看似随意，实际有明确规律：

1. 物体外轮廓比内部细节线粗；
2. 线条转角大的地方变粗，类似书法折角的顿笔；
3. 形状与形状的交界处变粗，体现遮挡关系；
4. 凹陷较深处变粗；
5. 阴影区域内的线条变粗；
6. 单条线的中段略粗于两端。
...
把这些规律归纳成可实现的维度，就是四类变化：**粗细**、**曲直**、**疏密**、**连续性**。它们分别服务于体积关系、穿插关系与远近空间感。

对应到实现上：

- 轮廓线与内部线分开控制宽度，内部线可以按重要度部分剔除；
- 小物体的线比大物体细，否则小物件会显得被墨线糊住；
- 转角处按曲率调整宽度——加粗表现浑厚，变细或断开表现尖锐，取决于风格；
- 阴影区域内适度加粗，与明暗分层配合；
- 线条允许在转角处断开。人眼会自动补完，观感往往优于处处闭合的线。

动画制作中有一个值得注意的现象：原画师的线条粗细变化在描原与中割环节会被逐步抹平，成片中的线条通常粗细相当均匀。因此"还原动画感"与"还原原画感"是两个不同目标，前者反而不需要复杂的宽度变化。

3D 渲染的优势在于品控稳定，可以按需要还原任一档位的线条表现。实现手段包括模型外扩、屏幕空间计算、贴图绘制与建模阶段预置，通常需要组合使用。
## Billboard 的朝向
