...
- 将正式文章归入 `知识库/`，将公开课程来源归入 `课程记录/`。
- 更新正式链接、来源台账、课程目标映射和验证脚本中的路径。
- 增加 Obsidian 正式知识、课程来源和排除课程来源的定向搜索方式。

### P1-foundations - 编程、网络与系统基础

- 扩展 Compute Shader 的资源类型、计数器、同步、回读、寄存器压力与 Wave 操作。
- 补齐 C++ 对象模型、虚函数、复制移动语义，以及 C# 装箱拆箱、事件、资源释放和异步基础。
- 补齐 `std::vector`、哈希表、红黑树与关联容器的结构、复杂度和失效规则。
- 新增操作系统聚合文章，覆盖进程线程、虚拟内存、并发原语、IPC、文件 IO 和计时。
- 扩展 DNS、TCP 连接管理、流量控制、拥塞控制与 TLS 基础。

### P2-coverage - 基础知识概念门禁

- 将课程概念台账扩展为 84 个高频概念组。
- 增加 Compute、C++/STL、C#、网络和操作系统的课程证据、检索别名与正式目标。
- 概念验证检查来源路径、目标文章、终态和正文术语命中。
