...
## 一句话定义
G-Buffer 是延迟渲染中存储几何阶段结果的多张屏幕空间缓冲。

## 为什么需要它

TA 需要理解 `G-Buffer`，因为它会影响资源制作、引擎配置、画面表现、调试路径或团队协作边界。把它写成明确条目，可以减少口头经验传递，并让问题排查有稳定入口。
...
在 Unity 和 Unreal 中落地 `G-Buffer` 时，TA 需要分别关注什么？

回答要点：比较两边的工具入口、资源规则、调试方式和平台限制，不要只背 API 名称。

### 问题 3

如果 `G-Buffer` 相关效果或资产在项目中出问题，你会怎么排查？

回答要点：从资源输入、引擎配置、运行时状态、性能指标和最小复现场景逐层缩小范围。

## 实践建议

- 为 `G-Buffer` 保留一个最小测试场景或示例资产，便于回归检查。
